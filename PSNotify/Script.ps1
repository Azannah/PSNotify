...
function Get-CacheItem {
  ...
  Begin {
    function Script:get_cache_file_path {
    ...
    function Script:read_cache_file {
      try {
        return Import-Clixml -Path Script:get_cache_file_path
      } catch {
        return $null
      }
    }
...
fuction Set-CacheItem {
  ...
  Process {
    ...
    try {
      $Value = Script:serialize_psobject $Value
    } catch {
    ...
    return Script:persist_cache_file $cache
  }
